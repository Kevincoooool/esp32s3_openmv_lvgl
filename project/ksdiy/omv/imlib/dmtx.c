...
#define DmtxAlmostZero          0.000001f
...
dmtxDecodeSetProp(DmtxDecode *dec, int prop, int value)
{
   switch(prop) {
      ...
      case DmtxPropSquareDevn:
         dec->squareDevn = cos(value * (M_PI/180.0f));
         break;
   ...
   if(dec->squareDevn <= 0.0f || dec->squareDevn >= 1.0f)
      return DmtxFail;
...
dmtxDecodeGetProp(DmtxDecode *dec, int prop)
{
   switch(prop) {
      ...
      case DmtxPropScanGap:
         return dec->scanGap;
      case DmtxPropSquareDevn:
         return (int)(acos(dec->squareDevn) * 180.0f/M_PI);
      case DmtxPropSymbolSize:
      ...
      DmtxVector2 p10, DmtxVector2 p11, DmtxVector2 p01)
{
   ...
   if(p00.X < 0.0f || p00.Y < 0.0f || p00.X > xMax || p00.Y > yMax ||
         p01.X < 0.0f || p01.Y < 0.0f || p01.X > xMax || p01.Y > yMax ||
         p10.X < 0.0f || p10.Y < 0.0f || p10.X > xMax || p10.Y > yMax)
      return DmtxFail;
   ...
   if(dimOT <= 8.0f || dimOR <= 8.0f || dimTX <= 8.0f || dimRX <= 8.0f)
      return DmtxFail;
   ...
   if(dmtxVector2Cross(&vOR, &vRX) <= 0.0f ||
         dmtxVector2Cross(&vOT, &vTX) >= 0.0f)
      return DmtxFail;
   ...
   color = colorTmp = 0;
    if (dec->image->channelCount == 1) // quicker for grayscale
    {
        ...
        for(i = 0; i < 5; i++) {
           ...
           p.X = (1.0f/symbolCols) * (symbolCol + sampleX[i]);
           p.Y = (1.0f/symbolRows) * (symbolRow + sampleY[i]);
 ...
 //                colorPlane, &colorTmp);
    ...
    else
    {
       for(i = 0; i < 5; i++) {
          ...
          err = dmtxDecodeGetPixelValue(dec, (int)(p.X + 0.5f), (int)(p.Y + 0.5f),
                colorPlane, &colorTmp);
          color += colorTmp;
       }
    }
...
MatrixRegionAlignCalibEdge(DmtxDecode *dec, DmtxRegion *reg, int edgeLoc)
{
   ...
   locOrigin.X = (int)(pTmp.X + 0.5f);
   locOrigin.Y = (int)(pTmp.Y + 0.5f);
   ...
   if(edgeLoc == DmtxEdgeTop) {
      ...
      pTmp.X = 0.8f;
      pTmp.Y = (symbolShape == DmtxSymbolRectAuto) ? 0.2f : 0.6f;
   }
   else {
      ...
      pTmp.X = (symbolShape == DmtxSymbolSquareAuto) ? 0.7f : 0.9f;
      pTmp.Y = 0.8f;
   }

   dmtxMatrix3VMultiplyBy(&pTmp, reg->fit2raw);
   loc1.X = (int)(pTmp.X + 0.5f);
   loc1.Y = (int)(pTmp.Y + 0.5f);
...
dmtxImageContainsFloat(DmtxImage *img, float x, float y)
{
   ...
   if(x >= 0.0f && x < (float)img->width && y >= 0.0f && y < (float)img->height)
      return DmtxTrue;
...
dmtxVector2Norm(DmtxVector2 *v)
{
  ...
  if(mag <= DmtxAlmostZero)
     return -1.0f; /* XXX this doesn't look clean */
...
dmtxDistanceFromRay2(const DmtxRay2 *r, const DmtxVector2 *q)
{
  ...
  assert(fabs(1.0f - dmtxVector2Mag(&(r->v))) <= DmtxAlmostZero);
...
#ifdef DEBUG
  /* Assumes that v is a unit vector */
  if(fabs(1.0f - dmtxVector2Mag(&(r->v))) > DmtxAlmostZero) {
...
void imlib_find_datamatrices(list_t *out, image_t *ptr, rectangle_t *roi, int effort)
{
    ...
    for (DmtxRegion *region = dmtxRegionFindNext(decode, max_iterations, &current_iterations); region; region = dmtxRegionFindNext(decode, max_iterations, &current_iterations)) {
        ...
        if (message) {
            ...
            p[0].X = p[0].Y = p[1].Y = p[3].X = 0.0f;
            p[1].X = p[3].Y = p[2].X = p[2].Y = 1.0f;
