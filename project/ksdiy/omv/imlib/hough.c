        ...
        case IMAGE_BPP_BINARY: {
            for (int y = roi->y + 1, yy = roi->y + roi->h - 1; y < yy; y += y_stride) {
                ...
                for (int x = roi->x + (y % x_stride) + 1, xx = roi->x + roi->w - 1; x < xx; x += x_stride) {
                    ...
                    int theta = fast_roundf((x_acc ? fast_atan2f(y_acc, x_acc) : 1.570796f) * 57.295780f) % 180; // * (180 / PI)
        ...
        while (list_size(out)) {
            ...
            for (size_t k = 0, l = list_size(out); k < l; k++) {
                ...
                if (theta_merge && rho_merge) {
                    ...
                    lnk_line.theta = fast_roundf(fast_atan2f(sin_mean, cos_mean) * 57.295780f) % 360; // * (180 / PI)
        ...
        case IMAGE_BPP_BINARY: {
            for (int y = roi->y + 1, yy = roi->y + roi->h - 1; y < yy; y += y_stride) {
                ...
                for (int x = roi->x + (y % x_stride) + 1, xx = roi->x + roi->w - 1; x < xx; x += x_stride) {
                    ...
                    int theta = fast_roundf((x_acc ? fast_atan2f(y_acc, x_acc) : 1.570796f) * 57.295780f) % 360; // * (180 / PI)
