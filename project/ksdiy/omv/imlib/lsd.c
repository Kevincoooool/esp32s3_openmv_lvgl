...
/** LSD Full Interface
    ...
    @param scale       When different from 1.0f, LSD will scale the input image
    ...
    @param sigma_scale When scale!=1.0f, the sigma of the Gaussian filter is:
    ...
    @param quant       Bound to the quantization error on the gradient norm.
                       ...
                       due to quantization will be bounded by 2.0f, as the
                       worst case is when the error are 1 and -1, that
                       gives an error of 2.0f.
                       Suggested value: 2.0f
    ...
    @param log_eps     Detection threshold, accept if -log10(NFA) > log_eps.
                       ...
                       intuitive than NFA:
                       - -1.0f gives an average of 10 false detections on noise
                       -  0.0f gives an average of 1 false detections on noise
                       -  1.0f gives an average of 0.1 false detections on nose
                       -  2.0f gives an average of 0.01 false detections on noise
                       .
                       Suggested value: 0.0f
...
/** Compute a Gaussian kernel of length 'kernel->dim',
    ...
    For example, if mean=0.5f, the Gaussian will be centered
...
static void gaussian_kernel(ntuple_list kernel, float sigma, float mean)
{
  ...
  if( sigma <= 0.0f ) error("gaussian_kernel: 'sigma' must be positive.");
                                      ...
                                      float sigma_scale )
{
  ...
  if( scale <= 0.0f ) error("gaussian_sampler: 'scale' must be positive.");
  if( sigma_scale <= 0.0f )
   ...
   */
  prec = 3.0f;
  h = (unsigned int) ceil( sigma * sqrt( 2.0f * prec * log(10.0f) ) );
  n = 1+2*h; /* kernel size */
  ...
  for(x=0;x<aux->xsize;x++)
    {
      ...
      /* coordinate (0.0f,0.0f) is in the center of pixel (0,0),
         so the pixel with xc=0 get the values of xx from -0.5f to 0.5f */
      xc = (int) floor( xx + 0.5f );
  ...
  for(y=0;y<out->ysize;y++)
    {
      ...
      yy = (float) y / scale;
      /* coordinate (0.0f,0.0f) is in the center of pixel (0,0),
         so the pixel with yc=0 get the values of yy from -0.5f to 0.5f */
      yc = (int) floor( yy + 0.5f );
                           ...
                           image_int * modgrad, unsigned int n_bins )
{
  ...
  if( threshold < 0.0f ) error("ll_angle: 'threshold' must be positive.");
    ...
    for(y=0;y<n-1;y++)
      {
        ...
        norm2 = gx*gx+gy*gy;
        norm = sqrt( norm2 / 4.0f ); /* gradient norm */
                      ...
                      float prec )
{
  ...
  if( prec < 0.0f ) error("isaligned: 'prec' must be positive.");
...
/** Computes the natural logarithm of the absolute value of
    ...
    @f[
      \Gamma(x) = \frac{ \sum_{n=0}^{N} q_n x^n }{ \Pi_{n=0}^{N} (x+n) }
                  (x+5.5)^{x+0.5f} e^{-(x+5.5)}
    @f]
    so
    @f[
      \log\Gamma(x) = \log\left( \sum_{n=0}^{N} q_n x^n \right)
                      + (x+0.5f) \log(x+5.5) - (x+5.5) - \sum_{n=0}^{N} \log(x+n)
    @f]
    and
      q0 = 75122.6331530,
      q1 = 80916.6278952,
...
/** Computes the natural logarithm of the absolute value of
    ...
    so
    @f[
        \log\Gamma(x) = 0.5f\log(2\pi) + (x-0.5f)\log(x) - x
...
static float log_gamma_windschitl(float x)
{
  return 0.918938533204673f + (x-0.5f)*log(x) - x
         + 0.5f*x*log( x*sinh(1/x) + 1/(810.0f*pow(x,6.0f)) );
}
...
//  static float inv[TABSIZE];   /* table to keep computed inverse values */
  ...
  log1term = log_gamma( (float) n + 1.0f ) - log_gamma( (float) k + 1.0f )
           ...
           + (float) k * log(p) + (float) (n-k) * log(1.0f-p);
  term = exp(log1term);

  /* in some cases no more computations are needed */
  if( double_equal(term,0.0f) )              /* the first term is almost zero */
    {
...
//                   1.0f / (float) i );
      bin_term = (float) (n-i+1) * ( 1.0f / (float) i );
      ...
      if(bin_term<1.0f)
        {
          ...
          err = term * ( ( 1.0f - pow( mult_term, (float) (n-i+1) ) ) /
                         (1.0f-mult_term) - 1.0f );
                         ...
                         image_int modgrad, float reg_angle, float prec )
{
  ...
  if( prec < 0.0f ) error("get_theta: 'prec' must be positive.");
  ...
  if( double_equal(Ixx,0.0f) && double_equal(Iyy,0.0f) && double_equal(Ixy,0.0f) )
    error("get_theta: null inertia matrix.");

  /* compute smallest eigenvalue */
  lambda = 0.5f * ( Ixx + Iyy - sqrt( (Ixx-Iyy)*(Ixx-Iyy) + 4.0*Ixy*Ixy ) );
                                 ...
                                 float density_th )
{
  ...
  if( prec < 0.0f ) error("reduce_region_radius: 'prec' must be positive.");
                   ...
                   image_char used, image_int angles, float density_th )
{
  ...
  if( prec < 0.0f ) error("refine: 'prec' must be positive.");
  ...
  tau = 2.0f * sqrt( (s_sum - 2.0f * mean_angle * sum) / (float) n
                               ...
                               int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  if( scale <= 0.0f ) error("'scale' value must be positive.");
  if( sigma_scale <= 0.0f ) error("'sigma_scale' value must be positive.");
  if( quant < 0.0f ) error("'quant' value must be positive.");
...
//  else
  ...
  */
  logNT = 5.0f * ( log10( (float) xsize ) + log10( (float) ysize ) ) / 2.0f
          + log10(11.0f);
...
//        degToRad(angles->data[ list_p->x + list_p->y * angles->xsize ]) != NOTDEF )
          ...
          because we are only interested in the exact NOTDEF value */
      {
        ...
        /*
           ...
           points with an offset of (0.5f,0.5f), that should be added to output.
...
void imlib_lsd_find_line_segments(list_t *out, image_t *ptr, rectangle_t *roi, unsigned int merge_distance, unsigned int max_theta_diff)
{
    ...
    int n_ls;
    float *ls = LineSegmentDetection(&n_ls, grayscale_image_tmp, roi->w, roi->h, 0.8, 0.6, 2.0f, 22.5, 0.0f, 0.7, 1024, NULL, NULL, NULL);
    ...
    for (int i = 0, j = n_ls; i < j; i++) {
        ...
        if(lb_clip_line(&lnk_line.line, 0, 0, roi->w, roi->h)) {
            ...
            lnk_line.theta = fast_roundf(rotation * 57.295780f) % 180; // * (180 / PI)
